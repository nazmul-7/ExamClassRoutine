<template>
    <div>
      	<div class="app-content  my-3 my-md-5">
					...
					<div class="col-md-12 col-lg-12">
						<div class="card">
							<div class="card-header card_header_area">
								<div class="card-title">Teacher Courses Data</div>
			...
			<Modal v-model="addModal" :mask-closable="false" :closable="false" title="Add New Teacher Courses">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Teacher Name</label>
                                <Select v-model="formItem.teacher_id"  placeholder="Please select a teacher" filterable>
			...
			<Modal :mask-closable="false" :closable="false" v-model="editModal"	title="Edit Teacher Courses">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Teacher Courses Department</label>
							...
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Teacher Courses Name</label>
...
export default {
	...
	methods : {
		...
		async add_category(){
            ...
            let tIndex = this.teacher_data.findIndex(d=>d.id == this.formItem.teacher_id)
            let cIndex = this.course_data.findIndex(d=>d.id == this.formItem.course_id)
            this.formItem.teacher_name = this.teacher_data[tIndex].name
			...
			this.loading = true
        	const res = await this.callApi('post',`app/admin/all_teacher_course/add`,this.formItem)
			if(res.status==201){
				this.addModal=false
				this.s('Teacher Courses added successfully!')
		...
		async edit_category(){
			...
			this.loading = true
        	const res = await this.callApi('post', 'app/admin/all_teacher_course/edit',this.edit_form)
			if(res.status==200){
				this.s('Teacher Courses updated successfully!')
		...
		async category_delete(id,index){
			if(!confirm("Are you sure to delete this Teacher Courses")){
				return;
			}
			let ob = {
				id:id
			...
			this.loading = true
			const res = await this.callApi('post',`app/admin/all_teacher_course/delete`,ob)
			if(res.status == 200){
				this.i(' Teacher Courses have been successfully Deleted!')
	...
	async created(){
	   this.loading = true
       const res = await this.callApi('get',`app/admin/all_teacher_course`)
		if(res.status == 200){
