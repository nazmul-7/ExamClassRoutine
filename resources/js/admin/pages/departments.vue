<template>
    <div>
      	<div class="app-content  my-3 my-md-5">
					...
					<div class="col-md-12 col-lg-12">
						<div class="card">
							<div class="card-header card_header_area">
								<div class="card-title">Departments Data</div>
												...
												<table id="example" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="example_info">
													<thead>
														<tr role="row">
															...
															<th class="wd-20p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 170px;">Department Name</th>
			...
			<Modal v-model="addModal" :mask-closable="false" :closable="false" title="Add New Department">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Department Name</label>
			...
			<Modal :mask-closable="false" :closable="false" v-model="editModal"	title="Edit Department">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							<div class="form-group">
...
export default {
	...
	methods : {
		...
		async add_category(){
			...
			this.loading = true
        	const res = await this.callApi('post',`app/admin/all_department/add`,this.formItem)
			if(res.status==201){
				this.addModal=false
				this.s('Department added successfully!')
		...
		async edit_category(){
			...
			this.loading = true
        	const res = await this.callApi('post', 'app/admin/all_department/edit',this.edit_form)
			if(res.status==200){
				this.s('Department updated successfully!')
		...
		async category_delete(id,index){
			...
			this.loading = true
			const res = await this.callApi('post',`app/admin/all_department/delete`,ob)
			if(res.status == 200){
				this.i(' Department have been successfully Deleted!')
	...
	async created(){
	   this.loading = true
       const res = await this.callApi('get',`app/admin/all_department`)
		if(res.status == 200){
