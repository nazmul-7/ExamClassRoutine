									...
									<div id="example_wrapper" class="dataTables_wrapper dt-bootstrap4 no-footer">
										<div class="row">
											<div class="col-sm-12 col-md-12">
												<div class="user_header_div mb-3">
													<div id="example_filter" class="dataTables_filter dataTables_filter_up">
														<label><input type="search" class="form-control form-control-sm" v-model="filterField" @change="searchByEmail"   placeholder="Search by email" aria-controls="example">
												...
												<table id="example" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="example_info">
													<thead>
														<tr role="row">
															...
															<th class="wd-15p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 101px;">Business Name</th>
															<th class="wd-15p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 101px;">City</th>
															<th class="wd-15p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 101px;">State</th>
															
															<th class="wd-15p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Start date: activate to sort column ascending" style="width: 101px;">User Type</th>
...
export default {
	...
	methods : {
		...
		async getpaginate(page = 1){
			const res  = await this.callApi('get',`app/all_user_pagi?page=${page}&total=${ parseInt(this.total)}`)
			if(res.status == 200){
				this.userData = res.data.data
				this.pagination = res.data
			}
		...
		async getAllData(){
			...
			this.loading = true
			const res = await this.callApi('get',`app/all_user_pagi?total=${this.total}`)
			if(res.status == 200){
