<template>
    <div>
      	<div class="app-content  my-3 my-md-5">
					...
					<div class="col-md-12 col-lg-12">
						<div class="card">
							<div class="card-header card_header_area">
								<div class="card-title">Semester Data</div>
												...
												<table id="example" class="table table-striped table-bordered dataTable no-footer" role="grid" aria-describedby="example_info">
													<thead>
														<tr role="row">
															...
															<th class="wd-20p sorting" tabindex="0" aria-controls="example" rowspan="1" colspan="1" aria-label="Position: activate to sort column ascending" style="width: 170px;"> Total Student</th>
			...
			<Modal v-model="addModal" :mask-closable="false" :closable="false" title="Add New Semester">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							...
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Total Student</label>
			...
			<Modal :mask-closable="false" :closable="false" v-model="editModal"	title="Edit Semester">
				<div class="card m-b-20">
					<div class="card-body">
						<form>
							<div class="form-group">
								<label class="form-label" for="exampleInputEmail1">Semester Department</label>
...
export default {
	...
	methods : {
		...
		async add_category(){
			...
			this.loading = true
        	const res = await this.callApi('post',`app/admin/all_semester/add`,this.formItem)
			if(res.status==201){
				this.addModal=false
				this.s('Semester added successfully!')
		...
		async edit_category(){
			...
			this.loading = true
        	const res = await this.callApi('post', 'app/admin/all_semester/edit',this.edit_form)
			if(res.status==200){
				this.s('Semester updated successfully!')
		...
		async category_delete(id,index){
			if(!confirm("Are you sure to delete this Semester")){
				return;
			}
			let ob = {
				id:id
			...
			this.loading = true
			const res = await this.callApi('post',`app/admin/all_semester/delete`,ob)
			if(res.status == 200){
				this.i(' Semester have been successfully Deleted!')
	...
	async created(){
	   this.loading = true
       const res = await this.callApi('get',`app/admin/all_semester`)
		if(res.status == 200){
